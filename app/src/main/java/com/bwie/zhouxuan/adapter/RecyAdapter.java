...
import com.bwie.zhouxuan.api.CustomJiaJian;
...
public class RecyAdapter extends RecyclerView.Adapter<RecyAdapter.MyViewHolder>{
     ...
     * 添加数据并更新显示
     * */
    public void add(SelectBean selectBean){
        //传进来的是bean对象
        if(list == null){
            list = new ArrayList<>();
        }
        //第一层遍历商家和商品
        for (SelectBean.DataBean shop : selectBean.getData()){
     ...
     * 设置数据源,控制是否显示商家
     * */
    private void setFirst(List<SelectBean.DataBean.ListBean> list) {
...
//        ImageLoader.getInstance().displayImage(split[0],holder.item_face);
        ...
        Uri uri;
        boolean inMemoryCache = imagePipeline.isInBitmapMemoryCache(Uri.parse(list.get(position).getImages()));
        DataSource<Boolean> inDiskCacheSource = imagePipeline.isInDiskCache(Uri.parse(list.get(position).getImages()));
        ...
        holder.item_delete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ApiService apiService = retrofit.create(ApiService.class);
                 Observable<DeleteBean> login = apiService.getDelete("983",""+list.get(position).getPid());
     ...
     * 计算总价的方法
     * */
    private void sum(List<SelectBean.DataBean.ListBean> list){
